...
def main():
    instance = DrugDiscoveryAi(config={
        "timeout": 30,
        "retries": 3,
        "debug": True,
    })

    print("=== Advanced drug-discovery-ai Example ===\n")

    # Batch processing
    inputs = [f"data_{i}" for i in range(10)]
    results = [instance.search(input=inp, index=i) for i, inp in enumerate(inputs)]
