"""FastAPI application for drug-discovery-ai."""
...
app = FastAPI(title="drug-discovery-ai", version="0.1.0", description="drug-discovery-ai API")
...
@app.get("/health")
def health():
    return {"status": "ok", "service": "drug-discovery-ai", "version": "0.1.0"}
...
async def process(req: ProcessRequest):
    ...
    try:
        ...
        resp = ProcessResponse(success=True, result=result, processing_time_ms=round(elapsed, 2),
                              metadata={"service": "drug-discovery-ai", "input_length": len(req.input)})
...
@app.get("/status")
def status():
    return {"service": "drug-discovery-ai", "version": "0.1.0", "ready": True,
